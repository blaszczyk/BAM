...
public class BAMSwingStoreTransaction extends BAMSwingFrame{
	...
	//Three Constructors
	private BAMSwingStoreTransaction( BAMUser user, BAMController controller )
	{
		super(controller);
		this.user = user;
	...
	public BAMSwingStoreTransaction( BAMUser user, BAMController controller, BAMTransaction transaction)
	{
	...
	public BAMSwingStoreTransaction( BAMUser user, BAMController controller, List<BAMTransaction> tList)
	{
		this(user,controller);
...
//				center.add( new JLabel( guiSettings.getPhrase("SUGG_MULTI") + suggestor.getSuggestion(0).getName() + " - " + suggestor.getSuggestion(0).getPurpose() ), SwingConstants.CENTER);
		...
		case TYPE_OLD_PAYMENT:
			...
			ptable.setButtonColumnMouse(3, select);
			ptable.draw();
